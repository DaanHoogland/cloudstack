...
public class StorageSystemDataMotionStrategy implements DataMotionStrategy {
    ...
    private void handleCreateTemplateFromSnapshot(SnapshotInfo snapshotInfo, TemplateInfo templateInfo, AsyncCompletionCallback<CopyCommandResult> callback) {
        ...
        try {
            ...
            try {
            ...
            catch (AgentUnavailableException | OperationTimedoutException ex) {
                throw new CloudRuntimeException("Failed to create template from snapshot : " + ex.getMessage());
            }
            finally {
                try {
                    ...
                    s_logger.warn("error during revoking access to snapshot: " + ex.getMessage(), ex);
                }
                ...
                try {
                    ...
                    s_logger.warn("error while processing snapshot event" + ex.getMessage(), ex);
                }
            }
    ...
    private void handleCreateVolumeFromTemplateBothOnStorageSystem(DataObject srcData, DataObject destData, AsyncCompletionCallback<CopyCommandResult> callback) {
        Preconditions.checkArgument(srcData != null, "Passing 'null' to srcData of handleCreateVolumeFromTemplateBothOnStorageSystem is not supported.");
        Preconditions.checkArgument(destData != null, "Passing 'null' to destData of handleCreateVolumeFromTemplateBothOnStorageSystem is not supported.");
        ...
        try {
            ...
            if (result.isFailed()) {
                s_logger.warn("Failed to create a volume: " + result.getResult());
    ...
    private HostVO getHost(Long zoneId, boolean computeClusterMustSupportResign) {
        ...
        for (HostVO host : hosts) {
            if (computeClusterMustSupportResign) {
                ...
                if (clusterDao.computeClusterSupportsResign(clusterId)) {
                    return host;
                }
                else {
    ...
    private CopyCmdAnswer performResignature(DataObject dataObj, HostVO hostVO, boolean keepGrantedAccess) {
        ...
        try {
            ...
            String msg = "Failed to revoke access to the volume from the hosts : ";
    ...
    private CopyCmdAnswer performCopyOfVdi(VolumeInfo volumeInfo, SnapshotInfo snapshotInfo, HostVO hostVO) {
        ...
        try {
            ...
            throw new CloudRuntimeException("Failed to perform VDI copy : " + ex.getMessage());
        }
        finally {
